...
@Component({
  ...
  styles: [`
    .support-fab {
      position: fixed; right: 90px; bottom: 22px; z-index: 80;
      width: 56px; height: 56px; border-radius: 50%;
      border: 1px solid rgba(200,169,74,0.5);
      background: linear-gradient(135deg, #c8a94a, #8a6d22);
      color: #060b14; font-size: 22px; cursor: pointer;
      box-shadow: 0 10px 30px rgba(0,0,0,0.45);
    }
    .fab-badge {
      position: absolute; top: -4px; right: -4px; background: #ce1126; color: #fff;
      font-size: 11px; min-width: 18px; height: 18px; border-radius: 50%;
      display: flex; align-items: center; justify-content: center; font-weight: 700;
    }
    .support-drawer {
      position: fixed; right: 22px; bottom: 88px; width: min(420px, calc(100vw - 32px));
      height: min(560px, calc(100vh - 140px)); z-index: 81;
      box-shadow: 0 20px 60px rgba(0,0,0,0.55);
    }
    @media (max-width: 640px) {
      .support-fab { right: 82px; bottom: 16px; }
      .support-drawer { right: 16px; bottom: 82px; }
    }
  `],
  template: `
    @if (open()) {
    ...
    <button type="button" class="support-fab" (click)="toggle()" title="Soporte">
      💬
      @if (!open() && unread()) {
