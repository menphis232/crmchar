...
async function migrate() {
  ...
  if (fs.existsSync(v7Path)) {
    ...
    const v37Path = path.join(__dirname, '..', 'sql', 'migration-v37-gestor-gallery.sql');
    if (fs.existsSync(v37Path)) {
      const v37 = fs.readFileSync(v37Path, 'utf8');
      const conn37 = await mysql.createConnection({
        host: process.env.DB_HOST || 'localhost',
        user: process.env.DB_USER || 'root',
        password: process.env.DB_PASSWORD || '',
        multipleStatements: true,
      });
      await conn37.query('USE tramites_vehiculares');
      for (const stmt of v37.split(';').map(s => s.trim()).filter(Boolean)) {
        try { await conn37.query(stmt); } catch (e) {
          if (!['ER_DUP_FIELDNAME'].includes(e.code)) throw e;
        }
      }
      await conn37.end();
    ...
    const v46Path = path.join(__dirname, '..', 'sql', 'migration-v46-vehicle-make-model-year.sql');
    if (fs.existsSync(v46Path)) {
      const v46 = fs.readFileSync(v46Path, 'utf8');
      const conn46 = await mysql.createConnection({
        host: process.env.DB_HOST || 'localhost',
        user: process.env.DB_USER || 'root',
        password: process.env.DB_PASSWORD || '',
        multipleStatements: true,
      });
      await conn46.query('USE tramites_vehiculares');
      for (const stmt of v46.split(';').map(s => s.trim()).filter(Boolean)) {
        try { await conn46.query(stmt); } catch (e) {
          if (!['ER_DUP_FIELDNAME', 'ER_TABLE_EXISTS_ERROR'].includes(e.code)) throw e;
        }
      }
      await conn46.end();
    ...
    const v48Path = path.join(__dirname, '..', 'sql', 'migration-v48-vehicle-insurance-tenencia.sql');
    if (fs.existsSync(v48Path)) {
      const v48 = fs.readFileSync(v48Path, 'utf8');
      const conn48 = await mysql.createConnection({
        host: process.env.DB_HOST || 'localhost',
        user: process.env.DB_USER || 'root',
        password: process.env.DB_PASSWORD || '',
        multipleStatements: true,
      });
      await conn48.query('USE tramites_vehiculares');
      for (const stmt of v48.split(';').map(s => s.trim()).filter(Boolean)) {
        try { await conn48.query(stmt); } catch (e) {
          if (!['ER_DUP_FIELDNAME', 'ER_TABLE_EXISTS_ERROR'].includes(e.code)) throw e;
        }
      }
      await conn48.end();
    ...
    const v57Path = path.join(__dirname, '..', 'sql', 'migration-v57-knowledge.sql');
    if (fs.existsSync(v57Path)) {
      const v57 = fs.readFileSync(v57Path, 'utf8');
      ...
      for (const stmt of v57.split(';').map(s => s.trim()).filter(Boolean)) {
        try { await conn57.query(stmt); } catch (e) {
          if (!['ER_TABLE_EXISTS_ERROR', 'ER_DUP_KEYNAME'].includes(e.code)) throw e;
        }
      }
      await conn57.end();
    ...
    const v59Path = path.join(__dirname, '..', 'sql', 'migration-v59-stripe-price-gestor.sql');
    if (fs.existsSync(v59Path)) {
      const v59 = fs.readFileSync(v59Path, 'utf8');
      const conn59 = await mysql.createConnection({
        host: process.env.DB_HOST || 'db',
        user: process.env.DB_USER || 'root',
        password: process.env.DB_PASSWORD || 'root',
        database: process.env.DB_NAME || 'tramites_vehiculares',
      ...
      for (const stmt of v59.split(';').map(s => s.trim()).filter(Boolean)) {
        try { await conn59.query(stmt); } catch (e) {
          if (!['ER_DUP_FIELDNAME', 'ER_TABLE_EXISTS_ERROR', 'ER_DUP_KEYNAME'].includes(e.code)) throw e;
        }
      }
      await conn59.end();
      console.log('Migración v59 (stripe price gestor) aplicada.');
    }
